...
async function runScan(forcedTime) {
  ...
  try {
    ...
    if (
      ...
      day.recommendations = buildRecommendations(day.scans);
    await saveToday(day);
    return day;
  } finally {
    scanning = false;
...
app.get("/api/gainers", async (_, res, next) => {
  try {
    const day = await getToday();
    res.json(day.scans.at(-1)?.gainers || (await fetchGainers()));
  } catch (e) {
    next(e);
  }
});
app.get("/api/losers", async (_, res, next) => {
  try {
    const day = await getToday();
    res.json(day.scans.at(-1)?.losers || (await fetchLosers()));
  } catch (e) {
    next(e);
  }
});
...
app.get("/api/history", async (_, res, next) => {
  try {
...
app.get("/api/history/:date", async (req, res, next) => {
  try {
    const day = await getDay(req.params.date);
